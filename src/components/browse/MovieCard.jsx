 ...
 *   disableHover — when true, suppresses MovieCardHover overlay entirely.
 *                  Used by Search page to avoid floating card overlap in the grid.
 */
const MovieCard = memo(({ item, mediaType = 'movie', isLarge = false, index = 0, disableHover = false }) => {
  ...
  // Hover overlay is disabled on search page (disableHover=true) to prevent
  // floating cards overlapping the grid layout.
  const enableHoverOverlay = isDesktop && !disableHover;
  ...
  return (
    <motion.div
      ...
      // Only activate hover animations on desktop, and only when not disabled
      variants={enableHoverOverlay ? cardHover : {}}
      initial="rest"
      whileHover={enableHoverOverlay ? 'hover' : undefined}
      animate="rest"
      onHoverStart={() => enableHoverOverlay && setHovered(true)}
      onHoverEnd={() => enableHoverOverlay && setHovered(false)}
      onClick={handleClick}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      {/* Hover overlay — desktop only, disabled when disableHover=true */}
      {enableHoverOverlay && hovered && (
        <MovieCardHover
          item={item}
